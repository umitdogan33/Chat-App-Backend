...
namespace Application.Common.Hubs
{
    public class MessageHub : Hub
    {
        ...
        public async Task SendMessageAsync(SendMessageDto messageDto)
        {
            ...
            MessageEntity messageEntity = new() {Id=Guid.NewGuid().ToString(),Message = messageDto.Message, ReceverId = data.UserId, SenderId = user };
            await _messageRepository.AddAsync(messageEntity);
            await Clients.Clients(data.ConnectionId,Context.ConnectionId).SendAsync("receiveMessage", messageDto.Message);
        }
